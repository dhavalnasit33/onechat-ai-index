    ...
    return (
      <div className="min-h-screen flex items-center justify-center font-sans">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-4">The requested category slug does not exist.</p>
      ...
      <div className="w-full max-w-[380px] md:max-w-full bg-white min-h-screen shadow-[0_4px_24px_rgba(0,0,0,0.12)] md:shadow-none">
          ...
          <div className="max-w-[1340px] mx-auto px-4 md:px-8 py-3.5 flex items-center justify-between">
            <div className="font-serif text-xs md:text-sm tracking-widest uppercase text-[#15151a] font-bold">
              <a href="/ai-behavior-index/">
                AI Behavior Index
                <span className="text-[#8a8a95] font-normal tracking-[0.04em] text-[10px] md:text-xs ml-0 md:ml-1.5 normal-case block md:inline mt-0.5 md:mt-0">by OneChat AI</span>
              </a>
            </div>

            {/* Desktop Links */}
            <nav className="hidden md:flex gap-7 font-sans text-sm text-[#4a4a55]">
              <a href="/ai-behavior-index/" className="hover:text-[#15151a] transition-colors">Explore</a>
        ...
        <div className="bg-white px-4 md:px-8 pt-3 md:pt-4">
          <div className="max-w-[1340px] mx-auto font-sans text-[11px] md:text-xs text-[#8a8a95] text-left">
            ...
            <span className="mx-1.5 md:mx-2 opacity-50">›</span>
            <span className="text-[#15151a] font-semibold">{category.name}</span>
          </div>
        </div>
        ...
        <section className="bg-white border-b border-[#d7e3f0] px-4 md:px-8 pt-6 pb-7 md:pt-10 md:pb-11 text-left">
          <div className="max-w-[1340px] mx-auto">
            ...
            <h1 className="font-serif text-[32px] md:text-[52px] leading-[1.05] font-bold tracking-[-0.02em] text-[#15151a] mb-3 md:mb-3.5 max-w-[880px]">
              Explore AI usage {category.name.toLowerCase().startsWith('by') ? '' : 'by '}
              <em className="italic text-[#088DFF] font-bold not-italic">{category.name.toLowerCase()}.</em>
            </h1>
            <p className="text-[13.5px] md:text-base text-[#4a4a55] max-w-[760px] leading-[1.55]">
              {category.description} Browse in-depth topic articles, each citing original research.
            </p>
          </div>
        </section>
        ...
        <div className="bg-white border-b border-[#d7e3f0] px-4 md:px-8 py-3.5 md:py-5 sticky top-[53px] md:top-[57px] z-15">
          <div className="max-w-[1340px] mx-auto flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
            ...
            {/* Search Box */}
            <form action={`/ai-behavior-index/${category.slug}/`} method="GET" className="relative w-full md:flex-1 md:max-w-[560px]">
            ...
            <div className="flex items-center gap-2.5 md:gap-3 w-full md:w-auto">
              ...
              <select
                id="sort-select"
                defaultValue={sort}
                className="font-sans text-xs md:text-[13px] text-[#15151a] bg-white border border-[#d7e3f0] rounded-full py-1.5 md:py-2 px-3 md:px-3.5 cursor-pointer font-medium outline-none hover:border-[#1e3a5f] flex-1 md:flex-none max-w-[140px] md:max-w-none"
              >
              ...
              <div className="font-sans text-[11px] md:text-xs text-[#8a8a95] ml-auto md:ml-auto whitespace-nowrap">
                <span className="hidden md:inline">Showing </span>
                <strong className="text-[#15151a] font-bold">
                  {totalCount === 0 ? 0 : skip + 1}–{Math.min(skip + limit, totalCount)}
                </strong> of <strong className="text-[#15151a] font-bold">{totalCount}</strong> <span className="hidden md:inline">topics</span>
              </div>
            </div>
        ...
        <main className="max-w-[1340px] mx-auto p-4 md:p-8">
          ...
          {topics.length === 0 ? (
            <div className="text-center py-20 text-[#8a8a95] font-sans">
              <p className="text-lg font-medium">No topics found matching your filters.</p>
            </div>
          ) : (
              ...
              {topics.map((topic: any, i: number) => (
                ...
                >
                  ...
                  <div className="font-sans text-[10.5px] text-[#8a8a95] tracking-[0.04em] pt-2.5 border-t border-[#eaf2fb] flex justify-between items-center">
                    ...
                    <span className="text-[#0468BD] font-bold text-[13px] leading-none">→</span>
                  </div>
                </a>
              ))}
            </div>
          ...
          <div className="mt-1 md:mt-2 mb-6 md:mb-8 p-[18px] md:p-[22px] md:px-[26px] bg-white border border-[#d7e3f0] rounded md:rounded-md text-left">
            <div className="font-sans text-[10px] tracking-[0.16em] uppercase text-[#8a8a95] font-bold mb-2">A note on methodology</div>
            <p className="text-[12.5px] md:text-[13px] text-[#4a4a55] leading-[1.55] max-w-[800px]">
              Every topic article links its statistics back to publicly available studies, surveys, and reports. We aggregate, organize, and contextualize this data — but the underlying research is conducted by the cited sources. This index is refreshed quarterly. If you run into any issues or have a study to suggest, contact us at <a href="mailto:support@onechatai.ai" className="text-[#4a4a55] underline">support@onechatai.ai</a>.
            </p>
          </div>
