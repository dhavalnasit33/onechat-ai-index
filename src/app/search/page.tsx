...
function highlightMatch(text: string, query: string): string {
  ...
  terms.forEach(term => {
    ...
    result = result.replace(re, '<mark class="bg-[#fff3a3] text-[#15151a] px-[2px] py-[1px] font-semibold rounded-[2px]">$1</mark>');
  });
  return result;
}
...
export default async function SearchPage({ searchParams }: PageProps) {
  ...
  // (Mongoose needs the model definition to populate relationships)
  ...
  if (q) {
    ...
    if (totalCount === 0) {
      ...
      topics = await Topic.find(fallbackFilter)
        .sort(sort === 'recent' ? { publishedAt: -1 } : sort === 'a-z' ? { title: 1 } : { dataPointsCount: -1 })
        .skip(skip)
        .limit(limit)
        .populate('categoryId', 'slug name')
        .lean();
  ...
  return (
    <div className="bg-[#f0f0f0] md:bg-white text-[#15151a] font-serif min-h-screen flex justify-center md:block">
      ...
      <div className="w-full max-w-[380px] md:max-w-full bg-white min-h-screen shadow-[0_4px_24px_rgba(0,0,0,0.12)] md:shadow-none">
          ...
          <div className="max-w-[1340px] mx-auto px-4 md:px-8 py-3.5 flex items-center justify-between">
            <div className="font-serif text-xs md:text-sm tracking-widest uppercase text-[#15151a] font-bold">
              <a href="/ai-behavior-index/">
                AI Behavior Index
                <span className="text-[#8a8a95] font-normal tracking-[0.04em] text-[10px] md:text-xs ml-0 md:ml-1.5 normal-case block md:inline mt-0.5 md:mt-0">
                  by OneChat AI
                </span>
              </a>
            </div>
            ...
            <nav className="hidden md:flex gap-7 font-sans text-sm text-[#4a4a55]">
              <a href="/ai-behavior-index/" className="hover:text-[#15151a] transition-colors">
                Explore
              </a>
              <a href="/ai-behavior-index/methodology/" className="hover:text-[#15151a] transition-colors">
                Methodology
              </a>
              <a href="/ai-behavior-index/for-journalists/" className="hover:text-[#15151a] transition-colors">
                For Journalists
              </a>
            </nav>
        ...
        {/* SEARCH HEADER */}
        <section className="bg-white border-b border-[#d7e3f0] px-4 md:px-8 pt-5 pb-4 md:pt-8 md:pb-[22px] text-left">
          <div className="max-w-[1340px] mx-auto">
            <div className="font-sans text-[10px] md:text-[11px] tracking-[0.18em] uppercase text-[#0468BD] font-bold mb-2.5 md:mb-3">
              Search results
            </div>
            <h1 className="font-serif text-[24px] md:text-[36px] leading-[1.15] md:leading-[1.1] font-normal tracking-[-0.015em] text-[#15151a] mb-1.5 md:mb-1">
              Results for{" "}
              <span className="font-bold text-[#0468BD]">"{q || 'All topics'}"</span>
            </h1>
            <p className="font-sans text-xs md:text-[13px] text-[#8a8a95]">
              <span className="hidden md:inline">Showing </span>
              <strong className="text-[#15151a] font-semibold">
                {totalCount === 0 ? 0 : skip + 1}–{Math.min(skip + limit, totalCount)}
              </strong> of{" "}
              <strong className="text-[#15151a] font-semibold">{totalCount}</strong>{" "}
              matching topics{" "}
        ...
        {/* SEARCH TOOLBAR */}
        <div className="bg-white border-b border-[#d7e3f0] px-4 md:px-8 py-3.5 md:py-[18px] sticky top-[53px] md:top-[57px] z-15">
          <div className="max-w-[1340px] mx-auto flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
            
            {/* Input Wrap */}
            <form action="/ai-behavior-index/search/" method="GET" className="relative w-full md:flex-1 md:max-w-[640px]">
              ...
              <input
                ...
                name="q"
                defaultValue={q}
                className="w-full font-sans text-sm text-[#15151a] bg-[#eaf2fb] border border-[#d7e3f0] rounded-full py-2.5 md:py-[11px] pr-10 pl-10 md:pl-[42px] md:pr-[44px] outline-none transition-colors focus:border-[#088DFF] focus:bg-white"
              ...
              {q && (
                <a
                  ...
                  className="absolute right-3 md:right-3.5 top-1/2 -translate-y-1/2 text-[#8a8a95] hover:bg-[#eaf2fb] hover:text-[#15151a] p-1 rounded-full transition-colors"
                >
            ...
            {/* Sort Dropdown */}
            <div className="flex items-center gap-2.5 md:gap-4 w-full md:w-auto">
              ...
              <select
                id="sort-select"
                defaultValue={sort}
                className="font-sans text-xs md:text-[13px] text-[#15151a] bg-white border border-[#d7e3f0] rounded-full py-1.5 md:py-2 px-3 md:px-3.5 cursor-pointer font-medium outline-none hover:border-[#1e3a5f] flex-1 md:flex-none md:max-w-[160px]"
              >
        ...
        {/* MAIN CONTENT */}
        <main className="max-w-[920px] mx-auto p-4 md:p-8">
          {/* RESULTS LIST */}
          <section className="mb-8">
            ...
            {topics.length === 0 ? (
              <div className="text-center py-20 text-[#8a8a95] font-sans">
                ...
                <p className="text-sm mt-1">Try searching for other keywords like "Gen Z", "Adoption", or "ChatGPT".</p>
              </div>
            ) : (
              <div className="flex flex-col">
                {topics.map((topic: any, i: number) => {
                  const categorySlug = topic.categoryId?.slug || 'unknown';
                  const categoryName = topic.categoryId?.name || 'Uncategorized';
                  return (
                    <a
                      key={topic._id.toString()}
                      href={`/ai-behavior-index/${categorySlug}/${topic.slug}/`}
                      className={`group grid grid-cols-[32px_1fr] md:grid-cols-[40px_1fr] gap-3 md:gap-[18px] py-4.5 md:py-[22px] border-b border-[#eaf2fb] ${i === topics.length - 1 ? "border-none" : ""} text-left`}
                    >
                      <div className="text-[18px] md:text-[22px] leading-none pt-1">
                        {icons[i % icons.length]}
                      </div>
                      <div className="min-w-0">
                        <div className="font-sans text-[9.5px] md:text-[10px] tracking-[0.14em] uppercase text-[#0468BD] font-bold mb-1 md:mb-1.5">
                          {categoryName}
                          <span className="text-[#8a8a95] font-normal mx-1.5 md:mx-2">
                            ·
                          </span>
                          <span className="text-[#8a8a95] font-medium">
                            <span className="hidden md:inline">Published </span>
                            {topic.publishedAt ? new Date(topic.publishedAt).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }) : 'Q2 2026'}
                          </span>
                        </div>
                        <h3
                          className="font-serif text-[18px] md:text-[22px] font-normal tracking-[-0.01em] text-[#15151a] leading-[1.2] mb-1.5 md:mb-2 transition-colors group-hover:text-[#0468BD]"
                          dangerouslySetInnerHTML={{ __html: highlightMatch(topic.title, q) }}
                        />
                        <p
                          className="font-sans text-[12.5px] md:text-[13.5px] text-[#4a4a55] leading-[1.55] mb-2.5 md:mb-3"
                          dangerouslySetInnerHTML={{ __html: highlightMatch(topic.description, q) }}
                        />
                        <div className="font-sans text-[10.5px] md:text-[11px] text-[#8a8a95] flex gap-3 md:gap-3.5 flex-wrap">
          ...
          {/* PAGINATION */}
          {totalPages > 1 && (
            <nav className="flex justify-center items-center gap-1 md:gap-1.5 py-6 md:py-7 pb-7 md:pb-10 font-sans">
              <a
                href={currentPage > 1 ? `?page=${currentPage - 1}&q=${q}&sort=${sort}` : '#'}
              ...
              <a
                href={currentPage < totalPages ? `?page=${currentPage + 1}&q=${q}&sort=${sort}` : '#'}
